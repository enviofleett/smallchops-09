...
export const useUpdateOrderStatus = (orderId: string): UseUpdateOrderStatusReturn => {
  ...
  const updateStatus = useCallback(async (newStatus: OrderStatus, notes?: string): Promise<boolean> => {
    ...
    try {
      ...
      // 🚚 DISABLED: Redundant email sending (database function already handles this)
      // The admin_update_order_status_bulletproof function automatically queues emails
      // Keeping this code for reference but commented out to prevent duplicate emails
      // 
