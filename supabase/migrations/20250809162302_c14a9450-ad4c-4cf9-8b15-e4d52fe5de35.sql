-- =====================================================
-- SAFE PAYMENT STATUS PROPAGATION - COMPATIBLE IMPLEMENTATION
-- =====================================================
-- This implementation works with your existing payment pipeline
-- without overriding any existing functions or triggers
...
CREATE OR REPLACE VIEW public.orders_with_payment 
SECURITY INVOKER AS
SELECT 
  o.*,
  -- Latest payment transaction details
...
BEGIN
  FOR order_rec IN order_cursor(p_order_id) LOOP
    ...
    new_order_status := CASE 
      ...
      ELSE order_rec.status -- Don't change if already confirmed/preparing/etc
    END;
    
    -- Update the order (this will trigger existing order update triggers)
    UPDATE orders 
    SET 
      payment_status = 'paid',
...
AS $$
BEGIN
  -- Orders with successful payments but wrong status
  RETURN QUERY
  SELECT 
  ...
  -- Unprocessed successful transactions
  RETURN QUERY
  SELECT 
...
BEGIN
  ...
  -- Process orders in batches to avoid long locks
  FOR order_rec IN (
    SELECT o.id
    FROM orders o
    WHERE EXISTS (
...
-- Only service_role can access the view
GRANT SELECT ON public.orders_with_payment TO service_role;

-- Grant access to system_logs based on RLS policies
GRANT SELECT ON public.system_logs TO authenticated; -- RLS will filter
GRANT INSERT ON public.system_logs TO service_role;

-- Step 10: Create helper function for admin UI to get dashboard stats
CREATE OR REPLACE FUNCTION public.get_payment_dashboard_stats()
RETURNS JSONB
SECURITY DEFINER
SET search_path TO 'public'
AS $$
...
BEGIN
  -- Get all health metrics as JSON
  FOR health_metrics IN 
    SELECT metric, value, description 
    FROM public.check_payment_system_health()
  LOOP
    stats := stats || jsonb_build_object(
      health_metrics.metric, 
      jsonb_build_object(
        'value', health_metrics.value,
        'description', health_metrics.description
      )
    );
  END LOOP;
  ...
  stats := stats || jsonb_build_object(
    'summary', jsonb_build_object(
      'needs_attention', (stats->'inconsistent_orders'->>'value')::INTEGER > 0,
      'recent_activity', (stats->'reconciliations_24h'->>'value')::INTEGER > 0,
      'pending_emails', (stats->'pending_emails'->>'value')::INTEGER,
      'last_checked', NOW()
    )
  );
...
GRANT EXECUTE ON FUNCTION public.get_payment_dashboard_stats() TO service_role;

-- Step 11: Add helpful comments for maintenance
