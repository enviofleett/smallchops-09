-- Finalize: create/replace functions and grants without touching existing view

-- Ensure system_logs table and RLS are present
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='system_logs' AND policyname='Admins can view system logs'
  ) THEN
    CREATE POLICY "Admins can view system logs" ON public.system_logs FOR SELECT USING (auth.jwt() ->> 'role' = 'admin' OR is_admin());
  END IF;
END $$;

DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE schemaname='public' AND tablename='system_logs' AND policyname='Service role can insert logs'
  ) THEN
    CREATE POLICY "Service role can insert logs" ON public.system_logs FOR INSERT WITH CHECK (auth.role() = 'service_role');
  END IF;
END $$;

CREATE INDEX IF NOT EXISTS idx_system_logs_event_type_created ON public.system_logs (event_type, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_communication_events_order_type_recent ON public.communication_events (order_id, event_type, created_at DESC);
...
DECLARE result BOOLEAN; BEGIN
  ...
  INTO result FROM orders o WHERE o.id=p_order_id; RETURN COALESCE(result,false); END; $$;
...
DECLARE start_time TIMESTAMPTZ; end_time TIMESTAMPTZ; processed_count INTEGER:=0; updated_count INTEGER:=0; order_rec RECORD; BEGIN
  ...
  orders_processed := processed_count; orders_updated := updated_count; processing_time_ms := EXTRACT(EPOCH FROM (end_time-start_time))*1000; RETURN NEXT; END; $$;
