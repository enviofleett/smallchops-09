-- Fix the create_order_with_items function to use text instead of non-existent payment_method enum
CREATE OR REPLACE FUNCTION public.create_order_with_items(p_customer_id uuid, p_fulfillment_type text, p_delivery_address jsonb DEFAULT NULL::jsonb, p_pickup_point_id uuid DEFAULT NULL::uuid, p_delivery_zone_id uuid DEFAULT NULL::uuid, p_guest_session_id uuid DEFAULT NULL::uuid, p_items jsonb DEFAULT '[]'::jsonb)
 RETURNS uuid
 LANGUAGE plpgsql
 SECURITY DEFINER
...
BEGIN
  ...
  -- Create the order - FIXED: Use text for payment_method instead of non-existent enum
  INSERT INTO orders (
    order_number,
    customer_id,
    customer_name,
    ...
    p_guest_session_id,
    v_total_amount,
    'pending',  -- FIXED: Use text literal instead of enum cast
    'pending',  -- FIXED: Use text literal instead of enum cast
    'pending',  -- FIXED: Use text literal instead of enum cast
    now(),
    now(),
