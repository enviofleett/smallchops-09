          ...
          <CardContent className="space-y-6">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-3 gap-2">
                {Array.from({
                length: 6
              }).map((_, i) => <div key={i} className="h-12 bg-muted animate-pulse rounded"></div>)}
              </div>
            </div>
          </CardContent>
        </Card>
    ...
    return <DeliverySchedulingErrorBoundary>
        <div className={cn("space-y-6", className)}>
          ...
          {calendarDate && selectedDateSlots.length > 0 && <div className="space-y-4">
              ...
              return <Button key={index} variant={isSelected ? "default" : "outline"} disabled={!timeSlot.available} onClick={() => handleTimeSlotSelect(timeSlot)} className={cn("h-14 px-3 flex items-center justify-center", "border-2 transition-all duration-200 hover:scale-105 active:scale-95", "text-center touch-manipulation rounded-lg", !timeSlot.available && "opacity-50 cursor-not-allowed", isSelected && "bg-primary text-primary-foreground border-primary shadow-md", !isSelected && "hover:bg-muted/50 hover:border-primary/20")}>
      ...
      <Card className={`border-primary/20 ${className}`}>
        ...
        <div className="space-y-4">
          ...
          <div className="p-3 bg-blue-50 border border-blue-200 rounded-lg">
            <p className="text-sm text-blue-800">
              ...
              {DELIVERY_BOOKING_CONSTANTS.BUSINESS_DAYS_ONLY && <span className="block mt-1">
          ...
          <div className="w-full overflow-hidden rounded-lg bg-background">
            <Calendar mode="single" selected={calendarDate} onSelect={handleDateSelect} disabled={isDateDisabled} fromDate={dateValidation.minDate} toDate={dateValidation.maxDate} className="w-full mx-auto rounded-lg border border-border/50 pointer-events-auto shadow-sm hover:shadow-md transition-shadow duration-200" classNames={{
              ...
              day: cn("h-10 w-10 mx-auto font-normal transition-all duration-200", "hover:bg-accent hover:text-accent-foreground rounded-lg", "focus:bg-accent focus:text-accent-foreground focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2", "aria-selected:opacity-100 touch-manipulation", "active:scale-95 disabled:pointer-events-none"),
          ...
          {calendarDate && <div className="text-sm text-muted-foreground bg-green-50 border border-green-200 rounded-lg p-3">
              ✅ Selected: <strong>{format(calendarDate, 'EEEE, MMMM d, yyyy')}</strong>
              {differenceInDays(calendarDate, new Date()) > 7 && <div className="mt-1 text-blue-600">
        ...
        {calendarDate && selectedDateSlots.length > 0 && <div className="space-y-4">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-3 gap-3">
              {selectedDateSlots.map((timeSlot, index) => <Button key={index} variant={selectedTimeSlot?.start_time === timeSlot.start_time && selectedTimeSlot?.end_time === timeSlot.end_time ? "default" : "outline"} disabled={!timeSlot.available} onClick={() => handleTimeSlotSelect(timeSlot)} className={cn("h-auto min-h-[72px] p-4 flex flex-col items-center justify-center gap-2 text-center", "transition-all duration-200 touch-manipulation rounded-xl border-2", "hover:scale-[1.02] active:scale-[0.98] focus:scale-[1.02]", "focus:outline-none focus:ring-2 focus:ring-primary focus:ring-offset-2", !timeSlot.available && "opacity-50 cursor-not-allowed hover:scale-100", timeSlot.available && "hover:shadow-md", selectedTimeSlot?.start_time === timeSlot.start_time && selectedTimeSlot?.end_time === timeSlot.end_time && "ring-2 ring-primary ring-offset-2 shadow-lg")}>
                  <span className="font-semibold text-sm leading-tight">
                    {timeSlot.start_time} - {timeSlot.end_time}
                  </span>
                  {!timeSlot.available && timeSlot.reason && <span className="text-xs text-destructive text-center leading-tight max-w-full">
                      {timeSlot.reason}
                    </span>}
                  {timeSlot.available && <span className="text-xs text-green-600 font-medium">Available</span>}
                </Button>)}
            </div>
          </div>}

        {/* No Slots Available Alert - Enhanced */}
        {calendarDate && selectedDateSlots.length === 0 && !selectedSlot?.is_holiday && <Alert className="border-orange-200 bg-orange-50">
        ...
        {selectedDate && selectedTimeSlot && <div className="pt-6 border-t border-border/50">
            ...
            <div className="space-y-4">
              <div className="flex flex-col sm:flex-row items-start sm:items-center gap-3">
                ...
                {differenceInDays(parseISO(selectedDate), new Date()) > 30 && <Badge variant="outline" className="px-3 py-1.5 text-sm font-medium whitespace-nowrap bg-blue-50 text-blue-700 border-blue-200">
                    🚀 Advanced Booking
                  </Badge>}
              </div>
        ...
        {selectedDate && differenceInDays(parseISO(selectedDate), new Date()) > 90 && <Alert className="border-blue-200 bg-blue-50">
