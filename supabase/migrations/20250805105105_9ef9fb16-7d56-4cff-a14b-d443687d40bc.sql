-- Fix UUID type mismatch in payment transaction debug functions
-- Update debug_payment_transaction_insert function to cast order_id to UUID
...
BEGIN
  ...
  RAISE LOG 'Debug insert attempt with params: order_id=%, email=%, amount=%, currency=%, method=%, type=%, status=%', 
    p_order_id, p_customer_email, p_amount, p_currency, p_payment_method, p_transaction_type, p_status;
  ...
  -- Validate input parameters before insert
  ...
  -- Attempt to insert the record with UUID casting
  BEGIN
    INSERT INTO payment_transactions (
      ...
      v_order_uuid,  -- Use the validated UUID
      p_customer_email,
      p_amount,
      p_currency,
      p_payment_method,
  ...
  EXCEPTION 
    WHEN check_violation THEN
      ...
      RETURN jsonb_build_object(
        ...
        'sqlstate', SQLSTATE,
        'sqlerrm', SQLERRM,
        'constraint_detail', 'One or more field values violate table constraints',
        'hint', 'Check that all enum values are valid and numeric values meet minimum requirements'
      );
...
-- Update minimal_payment_test_insert function to cast order_id to UUID
