-- Fix critical production issues (Drop and recreate function)
...
-- 3. Drop and recreate the database function to handle admin_notes properly
DROP FUNCTION IF EXISTS public.get_detailed_order_with_products(uuid);

CREATE FUNCTION public.get_detailed_order_with_products(p_order_id uuid)
RETURNS TABLE(
    id uuid, 
    order_number text, 
    customer_name text, 
