-- Fix RLS policy syntax error and complete Phase 1: Critical Security Fixes
...
-- 2. SECURE PAYMENT ACCESS CONTROL - FIX RLS POLICIES
-- Drop existing overly permissive policies
DROP POLICY IF EXISTS "Service role can manage payment transactions" ON payment_transactions;

-- Create secure granular policies with correct syntax
...
-- 3. SECURE COMMUNICATION EVENT ACCESS
-- Ensure communication events are properly secured
...
CREATE POLICY "Service role can create communication events" 
ON communication_events FOR INSERT 
WITH CHECK (auth.role() = 'service_role');

CREATE POLICY "Service role can update communication events" 
ON communication_events FOR UPDATE 
USING (auth.role() = 'service_role');

CREATE POLICY "Service role can read communication events for processing" 
ON communication_events FOR SELECT 
USING (auth.role() = 'service_role');

-- 4. SECURE ORDER ACCESS CONTROL
-- Ensure orders table has proper customer access controls
...
-- 5. CREATE PAYMENT SECURITY MONITORING FUNCTION
...
-- 6. IMPLEMENT SECURE API RATE LIMITING
...
-- 7. SECURE AUDIT LOG ACCESS
-- Ensure audit logs are properly protected
