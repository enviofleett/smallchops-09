...
-- Recreate function with correct parameter order: required first, then optional with defaults
...
BEGIN
    ...
    -- Calculate subtotal from items using consistent rounding
    ...
    IF p_promotion_code IS NOT NULL AND LENGTH(TRIM(p_promotion_code)) > 0 THEN
        ...
        IF v_promotion.id IS NOT NULL THEN
            ...
            IF v_promotion.min_order_amount IS NULL OR v_subtotal >= v_promotion.min_order_amount THEN
                -- Calculate discount based on promotion type with consistent rounding
    ...
    RAISE LOG 'Order created successfully: ID=%, Total=%, Items=%, Subtotal=%, Delivery=%, Discounts=%', 
        v_order_id, v_total_amount, jsonb_array_length(p_items), v_subtotal, v_delivery_fee, 
        (v_discount_amount + v_delivery_discount);

    RETURN v_order_id;

EXCEPTION WHEN OTHERS THEN
    -- Enhanced error logging for debugging
    RAISE LOG 'Order creation failed: % (SQLSTATE: %), Customer: %, Items: %', 
        SQLERRM, SQLSTATE, p_customer_id, jsonb_array_length(p_items);
    RAISE;
END;
$function$;