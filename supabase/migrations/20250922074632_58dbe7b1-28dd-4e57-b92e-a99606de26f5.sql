-- Fix the create_order_with_items function to use correct column and enum names
...
BEGIN
    ...
    -- Validate order_type (was fulfillment_type)
    IF p_fulfillment_type NOT IN ('delivery', 'pickup') THEN
        RAISE EXCEPTION 'Invalid order type. Must be delivery or pickup' USING ERRCODE = 'P0001';
    END IF;
    ...
    IF p_client_total IS NOT NULL THEN
        ...
        BEGIN
            IF v_difference_naira > v_tolerance_naira THEN
                ...
                RAISE LOG 'CALCULATION MISMATCH - Server: % naira (% cents), Client: % naira, Diff: % naira, Tolerance: % naira. Calculation log: %', 
    ...
    -- FIXED: Use order_type instead of fulfillment_type and cast to order_type enum
    INSERT INTO orders (
        customer_id,
        customer_email,
        customer_name,
        customer_phone,
        order_number,
        order_type,  -- CHANGED: was fulfillment_type
        delivery_address,
        pickup_point_id,
        delivery_zone_id,
        guest_session_id,
        ...
        v_customer_phone,
        v_order_number,
        p_fulfillment_type::order_type,  -- CHANGED: cast to order_type instead of order_fulfillment_type
        p_delivery_address,
        p_pickup_point_id,
        p_delivery_zone_id,
        p_guest_session_id,
    ...
    RAISE LOG 'Order created successfully: ID=%, Total=% naira (% cents), Items=%, Subtotal=% naira, Delivery=% naira, Discounts=% naira', 
...
EXCEPTION WHEN OTHERS THEN
    ...
    RAISE LOG 'Order creation failed: % (SQLSTATE: %), Customer: %, Items: %, Calculation log: %', 
