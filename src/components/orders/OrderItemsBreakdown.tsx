...
export function OrderItemsBreakdown({ 
  ...
  return (
    <Card className={`p-3 sm:p-6 ${className}`}>
      <div className="space-y-3 sm:space-y-4">
        <div className="flex items-center gap-2 mb-3 sm:mb-4">
          <ShoppingCart className="h-4 w-4 sm:h-5 sm:w-5 text-primary" />
          <h3 className="text-base sm:text-lg font-semibold">Order Items ({items.length})</h3>
        </div>

        {/* Items List */}
        <div className="space-y-2 sm:space-y-3">
          {items.map((item, index) => (
            <div key={item.id || index} className="border border-border rounded-lg p-3 sm:p-4 bg-card/50 hover:bg-card/80 transition-colors">
              <div className="flex flex-col sm:flex-row justify-between items-start gap-3 sm:gap-4">
                <div className="flex-1 w-full">
                  <div className="flex items-start gap-2 sm:gap-3">
                    {item.product?.image_url ? (
                      <div className="w-10 h-10 sm:w-12 sm:h-12 rounded-lg overflow-hidden flex-shrink-0 ring-1 ring-primary/20">
                        <img 
                      ...
                      <div className="w-10 h-10 sm:w-12 sm:h-12 bg-primary/10 rounded-lg flex items-center justify-center flex-shrink-0 ring-1 ring-primary/20">
                        <Package className="h-4 w-4 sm:h-5 sm:w-5 text-primary" />
                      </div>
                    )}
                    <div className="flex-1 min-w-0">
                      <div className="flex flex-col sm:flex-row sm:items-start justify-between gap-2">
                        <h4 className="font-semibold text-foreground break-words text-sm sm:text-base">{item.product_name}</h4>
                        
                        {/* Mobile Price - Show at top right on mobile */}
                      ...
                      {showDetailed && renderWhatsIncluded(item) && (
                        <div className="mb-2 sm:mb-3 p-2 sm:p-3 bg-primary/5 rounded-lg border border-primary/20">
                          <div className="flex items-start gap-2">
                            <Package className="h-3 w-3 sm:h-4 sm:w-4 text-primary mt-0.5 flex-shrink-0" />
                            <div className="flex-1">
                              <h5 className="text-xs sm:text-sm font-semibold text-primary mb-1 sm:mb-2">What's Included:</h5>
                      ...
                      {showDetailed && formatCustomizations(item.customizations) && (
                        <div className="mb-2 p-2 bg-accent/30 rounded border-l-2 border-accent">
                          <p className="text-xs sm:text-sm text-accent-foreground flex items-start gap-1">
                      ...
                      {showDetailed && item.special_instructions && (
                        <div className="mb-2 p-2 bg-orange-50 border border-orange-200 rounded">
                          <p className="text-xs sm:text-sm text-orange-700 flex items-start gap-1">
                      ...
                      {/* Product Details Row - Mobile Optimized */}
                      <div className="flex flex-wrap items-center gap-3 sm:gap-4 mt-2 text-xs sm:text-sm text-muted-foreground">
                        ...
                        <div className="flex items-center gap-1">
                          ...
                          <span className="text-foreground font-medium">{formatCurrency(Number(item.unit_price ?? 0))}</span>
                        </div>
        ...
        <div className="space-y-2 sm:space-y-3">
          ...
          <div className="space-y-2">
            <div className="flex justify-between text-sm">
              <span className="text-muted-foreground">Items Subtotal ({items.length} items)</span>
          ...
          <div className="bg-primary/5 p-3 sm:p-4 rounded-lg border border-primary/20">
            <div className="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-2">
              <span className="text-base sm:text-lg font-semibold text-foreground">Order Total</span>
              <span className="text-xl sm:text-2xl font-bold text-primary">{formatCurrency(grandTotal)}</span>
            </div>
