-- Create promotion code rate limiting table (corrected)
CREATE TABLE public.promotion_code_rate_limits (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  identifier TEXT NOT NULL, -- IP address or user identifier
  window_hour TIMESTAMP WITH TIME ZONE NOT NULL DEFAULT date_trunc('hour', NOW()),
...
CREATE UNIQUE INDEX idx_promotion_rate_limits_identifier_hour 
ON public.promotion_code_rate_limits (identifier, window_hour);

-- Create index for cleanup operations
CREATE INDEX idx_promotion_rate_limits_window_hour 
ON public.promotion_code_rate_limits (window_hour);
...
-- Function to check promotion code rate limiting (corrected)
...
BEGIN
  ...
  IF v_rate_limit_record IS NULL THEN
    INSERT INTO public.promotion_code_rate_limits (identifier, window_hour, attempt_count)
