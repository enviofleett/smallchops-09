-- Fix the create_order_with_items function that process-checkout calls
-- This function has the signature: (p_customer_id uuid, p_fulfillment_type text, p_items jsonb, ...)
...
BEGIN
    ...
    -- FIXED: Create order record with correct column names that exist in orders table
    INSERT INTO orders (
        customer_id,
        customer_email,
        customer_name,
        ...
        status,
        payment_status
        -- REMOVED: discount_amount, delivery_discount, promotion_code (don't exist in orders table)
    ) VALUES (
        p_customer_id,
        v_customer_email,
        v_customer_name,
