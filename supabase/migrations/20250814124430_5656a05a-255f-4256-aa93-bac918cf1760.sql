-- Add indexes for delivery management performance
...
-- Add delivery performance tracking table
...
-- Trigger to update metrics when orders are updated
CREATE OR REPLACE FUNCTION update_delivery_metrics_trigger()
RETURNS TRIGGER AS $$
BEGIN
...
CREATE TRIGGER update_delivery_metrics_on_order_change
  AFTER UPDATE ON orders
  FOR EACH ROW
  WHEN (OLD.status IS DISTINCT FROM NEW.status)
  EXECUTE FUNCTION update_delivery_metrics_trigger();