-- CRITICAL SECURITY FIX: Enable RLS and add policies for exposed tables (CORRECTED)
...
-- 5. Fix payment_transactions table (uses order_id, not customer_id)
...
-- Allow customers to view their own payment transactions through order relationship
...
-- 6. Fix payment_intents table (uses order_id, not customer_id)
...
-- Allow customers to view their own payment intents through order relationship
...
VALUES (
  'security_hardening_applied_v3',
  'Security Audit',
  'Critical RLS policies applied to protect sensitive data (CORRECTED)',
  jsonb_build_object(
    ...
    'column_mappings', jsonb_build_object(
      'payment_transactions', 'order_id -> orders.customer_id',
      'payment_intents', 'order_id -> orders.customer_id'
    )
  )
);