-- Create promotion code rate limiting table
CREATE TABLE public.promotion_code_rate_limits (
...
CREATE UNIQUE INDEX idx_promotion_rate_limits_identifier_window 
ON public.promotion_code_rate_limits (identifier, date_trunc('hour', window_start));

-- Create index for cleanup operations
CREATE INDEX idx_promotion_rate_limits_window_start 
ON public.promotion_code_rate_limits (window_start);

-- Enable RLS
ALTER TABLE public.promotion_code_rate_limits ENABLE ROW LEVEL SECURITY;

-- RLS Policy: Only service roles can manage rate limits
CREATE POLICY "Service roles can manage promotion rate limits"
ON public.promotion_code_rate_limits
FOR ALL
USING (auth.role() = 'service_role')
WITH CHECK (auth.role() = 'service_role');

-- Create enhanced promotion security audit table
CREATE TABLE public.promotion_security_audit (
  id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
  promotion_id UUID REFERENCES public.promotions(id),
  customer_email TEXT,
  customer_id UUID,
  action_type TEXT NOT NULL, -- 'attempt', 'success', 'failure', 'blocked'
  failure_reason TEXT,
  ip_address INET,
  user_agent TEXT,
...
CREATE INDEX idx_promotion_security_audit_created_at 
ON public.promotion_security_audit (created_at);

CREATE INDEX idx_promotion_security_audit_action_type 
ON public.promotion_security_audit (action_type);

CREATE INDEX idx_promotion_security_audit_ip_address 
ON public.promotion_security_audit (ip_address);

-- Enable RLS
ALTER TABLE public.promotion_security_audit ENABLE ROW LEVEL SECURITY;

-- RLS Policies for security audit
CREATE POLICY "Admins can view promotion security audit"
ON public.promotion_security_audit
FOR SELECT
USING (is_admin());

CREATE POLICY "Service roles can manage promotion security audit"
ON public.promotion_security_audit
FOR ALL
USING (auth.role() = 'service_role')
WITH CHECK (auth.role() = 'service_role');

-- Function to check promotion code rate limiting
...
BEGIN
  ...
  FROM public.promotion_code_rate_limits
  WHERE identifier = p_identifier
  AND date_trunc('hour', window_start) = v_current_window;
  ...
  IF v_rate_limit_record IS NULL THEN
    INSERT INTO public.promotion_code_rate_limits (identifier, window_start, attempt_count)
    VALUES (p_identifier, v_current_window, 1)
    ON CONFLICT (identifier, date_trunc('hour', window_start))
  ...
  IF v_rate_limit_record.attempt_count >= p_max_attempts THEN
    ...
    UPDATE public.promotion_code_rate_limits
...
-- Function to validate and apply promotion code securely
...
BEGIN
  ...
  IF p_code IS NULL OR trim(p_code) = '' THEN
    -- Log security audit
    INSERT INTO public.promotion_security_audit (
...
BEGIN
  ...
  DELETE FROM public.promotion_code_rate_limits
