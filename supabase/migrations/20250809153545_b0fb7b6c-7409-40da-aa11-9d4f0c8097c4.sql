-- 1) Ensure required extensions for cron and http
CREATE EXTENSION IF NOT EXISTS pg_cron;
CREATE EXTENSION IF NOT EXISTS pg_net;

-- 2) Attach triggers on orders
ALTER TABLE public.orders ENABLE ROW LEVEL SECURITY;

-- BEFORE UPDATE: set_paid_at_on_status_change
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'orders_set_paid_at_on_status_change'
  ) THEN
...
-- AFTER INSERT OR UPDATE: trigger_order_emails (includes payment confirmation)
DO $$
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'orders_trigger_order_emails'
  ) THEN
...
-- 3) Payment processing trigger on payment_transactions
...
AS $$
BEGIN
  -- Only process on success/paid or when paid_at is set, and avoid re-processing
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger WHERE tgname = 'payment_tx_process_success' 
  ) THEN
...
-- 5) View: orders_with_payment (latest tx + derived fields)
...
-- 6) RLS policies for SELECT on orders and payment_transactions
-- Admins can select all
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Admins can select all orders'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Admins can select all payment tx'
  ) THEN
...
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Customers can select their orders'
  ) THEN
...
-- Customers can select their own payment transactions (by order ownership or matching payment_reference)
DO $$ BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_policies WHERE polname = 'Customers can select their payment tx'
  ) THEN
...
-- 8) Cron to reconcile pending orders every 5 minutes
-- Unschedule if exists, then schedule
SELECT cron.unschedule('reconcile-payments-every-5-mins') WHERE EXISTS (
  SELECT 1 FROM cron.job WHERE jobname = 'reconcile-payments-every-5-mins'
);
