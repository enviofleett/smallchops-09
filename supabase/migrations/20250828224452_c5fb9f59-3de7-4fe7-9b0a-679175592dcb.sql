-- Fix critical database issues from audit report (corrected syntax)
...
DO $$
BEGIN
  IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'chk_orders_total_amount_positive') THEN
...
-- 8. Create trigger for order audit logging
...
-- 9. Create trigger
DROP TRIGGER IF EXISTS order_changes_audit_trigger ON orders;
CREATE TRIGGER order_changes_audit_trigger
