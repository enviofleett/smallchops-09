-- Enhanced branding system database setup - Fixed version
...
CREATE TABLE IF NOT EXISTS public.upload_rate_limits (
  ...
  UNIQUE(user_id, date_trunc('hour', window_start))
);
...
BEGIN
  ...
  AND date_trunc('hour', window_start) = date_trunc('hour', NOW());
  
  IF v_count IS NULL THEN
    INSERT INTO public.upload_rate_limits (user_id, upload_count, window_start)
    VALUES (p_user_id, 1, date_trunc('hour', NOW()));
    RETURN TRUE;
