-- CRITICAL PHASE 1: Emergency Security Fixes - Fixed Version
...
-- Create rate limits table for distributed rate limiting (fixed structure)
...
CREATE INDEX IF NOT EXISTS idx_enhanced_rate_limits_window ON public.enhanced_rate_limits(window_end);
...
-- RLS policies for enhanced rate limits (service role only)
CREATE POLICY "Service role can manage enhanced rate limits" ON public.enhanced_rate_limits
  FOR ALL USING (auth.role() = 'service_role');