-- Phase 1 & 2: Enhanced Email System Activation & Automated Processing
...
-- Create function for automated email queue processing
CREATE OR REPLACE FUNCTION public.process_email_queue_batch(batch_size INTEGER DEFAULT 50)
RETURNS JSONB AS $$
DECLARE
...
BEGIN
  ...
  LOOP
    BEGIN
      ...
      -- Invoke enhanced email processor
      SELECT net.http_post(
        url := 'https://oknnklksdiqaifhxaccs.supabase.co/functions/v1/enhanced-smtp-sender',
        headers := '{"Content-Type": "application/json", "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9rbm5rbGtzZGlxYWlmaHhhY2NzIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MzE5MDkxNCwiZXhwIjoyMDY4NzY2OTE0fQ.P0dfgKXjnWKR3r5Mee_7d7YWqKMQJsGiM3EbNh4Eb08"}'::jsonb,
        body := jsonb_build_object('event_id', queue_record.event_id)
      ) INTO result;
...
SELECT cron.schedule(
  'process-enhanced-email-queue',
  '*/30 * * * * *', -- Every 30 seconds
  $$
  SELECT public.process_email_queue_batch(50);
  $$
);

-- Process high priority emails every 10 seconds
SELECT cron.schedule(
  'process-high-priority-emails',
  '*/10 * * * * *', -- Every 10 seconds
  $$
  SELECT net.http_post(
    url := 'https://oknnklksdiqaifhxaccs.supabase.co/functions/v1/instant-email-processor',
    headers := '{"Content-Type": "application/json", "Authorization": "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6Im9rbm5rbGtzZGlxYWlmaHhhY2NzIiwicm9sZSI6InNlcnZpY2Vfcm9sZSIsImlhdCI6MTc1MzE5MDkxNCwiZXhwIjoyMDY4NzY2OTE0fQ.P0dfgKXjnWKR3r5Mee_7d7YWqKMQJsGiM3EbNh4Eb08"}'::jsonb,
    body := '{"priority": "high"}'::jsonb
  );
  $$
);
...
SELECT cron.schedule(
  ...
  '0 2 * * *', -- Daily at 2 AM
  $$
  SELECT public.cleanup_email_processing_data();
  $$
);
