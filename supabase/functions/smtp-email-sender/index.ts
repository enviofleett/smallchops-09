...
async function sendSMTPEmail(config: any, emailData: any) {
  ...
  try {
    // Enhanced connection logic with timeout and better error handling
    console.log(`Attempting connection to ${host}:${port} (${secure ? 'SSL' : 'STARTTLS'})`);
    ...
    // STARTTLS for non-secure connections (primarily port 587)
    if (!secure && port === 587) {
...
serve(async (req) => {
  ...
  try {
    ...
    // Enhanced SMTP configuration with port optimization
    console.log('Current SMTP port:', settings.smtp_port);
    
    // Default to port 587 (STARTTLS) for better compatibility
    let smtpPort = settings.smtp_port || 587;
    let smtpSecure = settings.smtp_secure !== false;
    
    // Optimize port and security settings
    if (smtpPort === 465) {
      smtpSecure = true; // Port 465 always uses SSL
      console.log('Using port 465 with SSL');
    } else if (smtpPort === 587) {
      smtpSecure = false; // Port 587 uses STARTTLS
      console.log('Using port 587 with STARTTLS');
    }
