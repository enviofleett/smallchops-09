-- PHASE 1: FIX CRITICAL DATABASE SCHEMA ISSUES (CORRECTED)
...
-- 3. SECURITY HARDENING: Restrict public access to sensitive tables
...
-- 4. Fix products table image reference issue
-- Add index for performance if not exists
CREATE INDEX IF NOT EXISTS idx_products_image_url ON products(image_url);

-- 5. Create a fixed function for order details that uses correct column
