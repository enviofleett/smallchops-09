-- Add email deduplication to prevent spam

-- Step 1: Update trigger_order_status_email() with deduplication logic
...
BEGIN
  ...
  IF TG_OP = 'UPDATE' AND OLD.status IS DISTINCT FROM NEW.status THEN
    ...
    -- Determine template key (no NULL values)
    v_template_key := CASE 
...
-- Step 2: Clean up existing queued events with NULL template_key
UPDATE communication_events
SET status = 'failed',
    error_message = 'Invalid template_key: NULL - cleaned up during deduplication migration',
    updated_at = NOW()
WHERE template_key IS NULL
  AND status = 'queued';

-- Step 3: Add index on dedupe_key for faster lookups
CREATE INDEX IF NOT EXISTS idx_communication_events_dedupe_key 
ON communication_events(dedupe_key, created_at DESC);

-- Step 4: Add index on recipient_email and sent_at for rate limiting
CREATE INDEX IF NOT EXISTS idx_communication_events_rate_limit 
