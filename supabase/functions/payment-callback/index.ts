...
async function processVerifiedPayment(supabase: any, reference: string, paystackData: any) {
  try {
    ...
    log('info', '✅ RPC operation successful', {
      ...
      has_customer_email: !!orderData.customer_email
    });

    // Derive robust recipient email from multiple sources
    let recipientEmail = orderData.customer_email;
    
    // Fallback 1: Get from orders table if RPC didn't return it
    if (!recipientEmail) {
      log('info', '🔍 RPC missing customer_email, fetching from orders table...');
      try {
        ...
        if (orderDetails?.customer_email) {
          ...
          log('info', '✅ Found customer_email from orders table');
        } else {
          log('warn', '⚠️ No customer_email in orders table:', orderError?.message);
        }
      } catch (fetchError) {
        log('warn', '⚠️ Failed to fetch customer_email from orders:', fetchError.message);
      }
    }
    
    // Fallback 2: Extract from Paystack data if available
    if (!recipientEmail && paystackData?.customer?.email) {
      recipientEmail = paystackData.customer.email;
      log('info', '✅ Using customer_email from Paystack data');
    }
    ...
    if (!isValidEmail) {
      log('error', '❌ No valid recipient email found for payment confirmation', {
        from_rpc: orderData.customer_email,
        from_orders: undefined, // will be logged above if fetched
        from_paystack: paystackData?.customer?.email,
        order_id: orderData.order_id,
        reference
      });
      recipientEmail = null; // Explicitly set to null for consistency
    } else {
      recipientEmail = recipientEmail.trim().toLowerCase();
      log('info', '✅ Valid recipient email confirmed', { 
        recipientEmail: recipientEmail.substring(0, 3) + '***@' + recipientEmail.split('@')[1] 
      });
    }
    ...
    if (recipientEmail) {
      log('info', '📧 Queuing payment confirmation email via processor', {
        order_id: orderData.order_id,
        order_number: orderData.order_number,
        customer_email: recipientEmail,
        amount: orderData.amount || paystackAmount
      });
      
      try {
        const processorResult = await supabase.functions.invoke('payment-confirmation-processor', {
          body: {
            order_id: orderData.order_id,
            customer_email: recipientEmail,
            customer_name: orderData.customer_name || 'Valued Customer',
            amount: orderData.amount || paystackAmount,
            payment_reference: reference || 'Paystack'
          }
        });

        if (processorResult.error) {
          log('error', '❌ Payment confirmation processor failed', {
            error: processorResult.error.message || 'Unknown error',
            order_id: orderData.order_id,
            recipient_email: recipientEmail,
            reference: reference
          });
          
          // Log for monitoring but don't block payment success
          await supabase.from('audit_logs').insert({
            action: 'payment_confirmation_processor_error',
            category: 'Email Processing',
            message: `Payment confirmation processor failed for order ${orderData.order_number}`,
            new_values: {
              order_id: orderData.order_id,
          ...
          if (processorData?.already_sent) {
            log('info', '✅ Payment confirmation already sent for this order', {
              order_id: orderData.order_id,
              order_number: orderData.order_number
            });
          } else {
            log('info', '✅ Payment confirmation queued successfully', {
              order_id: orderData.order_id,
              order_number: orderData.order_number,
              email_event_id: processorData?.email_event_id,
              customer_email: processorData?.customer_email
            });
          }
        }
      } catch (processorError) {
        log('error', '❌ Error invoking payment-confirmation-processor', {
          error: processorError.message,
          order_id: orderData.order_id,
          reference: reference
        });
        
        // Log to audit for monitoring but don't block payment success
        await supabase.from('audit_logs').insert({
          action: 'payment_confirmation_processor_exception',
          category: 'Email Processing',
          message: `Exception invoking payment-confirmation-processor for order ${orderData.order_number}`,
          new_values: {
            order_id: orderData.order_id,
            error: processorError.message,
            reference: reference
      ...
      log('warn', '⚠️ Skipping payment confirmation email - no valid recipient email available', {
        order_id: orderData.order_id,
        reference: reference
      });
    }
