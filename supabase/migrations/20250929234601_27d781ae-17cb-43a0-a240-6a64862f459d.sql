-- Fix RPC JSON casting - SIMPLIFIED VERSION
-- Remove nested CASE that causes premature casting errors
...
BEGIN
    ...
    -- Get order items - SIMPLIFIED INGREDIENTS HANDLING
    SELECT jsonb_agg(
        jsonb_build_object(
            'id', oi.id,
...
VALUES (
    'rpc_json_casting_fixed_v3_simplified',
    'Database Optimization',
    'Simplified ingredients casting to avoid nested CASE errors',
    jsonb_build_object(
        'issue', 'Nested CASE statement causing premature casting errors',
        'fix', 'Single COALESCE wrapping entire CASE statement'
    )
);