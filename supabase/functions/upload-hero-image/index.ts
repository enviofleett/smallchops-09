...
interface HeroImageUploadRequest {
  file: {
    name: string;
    type: string;
    size: number;
...
Deno.serve(async (req) => {
  ...
  console.log('Hero image upload request:', req.method, req.url, 'from origin:', origin);
  ...
  try {
    ...
    if (req.method === 'POST') {
      const body: HeroImageUploadRequest = await req.json();
      ...
      const fileName = `hero-${timestamp}-${randomId}.${fileExtension}`;
      ...
      await supabase
        ...
        .insert({
          ...
          category: 'Branding',
          message: `Hero image uploaded: ${file.name}`,
          new_values: {
            file_name: file.name,
            file_size: file.size,
            file_type: file.type,
