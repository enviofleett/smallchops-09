-- Create missing tables and indexes only (avoiding duplicate policies)
...
DO $$ BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'webhook_logs' AND policyname = 'Service roles can manage webhook logs') THEN
    CREATE POLICY "Service roles can manage webhook logs" ON webhook_logs
    FOR ALL USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'transaction_analytics' AND policyname = 'Admins can view analytics') THEN
  ...
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'transaction_analytics' AND policyname = 'Service roles can manage analytics') THEN
    CREATE POLICY "Service roles can manage analytics" ON transaction_analytics
    FOR ALL USING (auth.role() = 'service_role')
    WITH CHECK (auth.role() = 'service_role');
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'customer_payment_preferences' AND policyname = 'Users can manage own preferences') THEN
    CREATE POLICY "Users can manage own preferences" ON customer_payment_preferences
    FOR ALL USING (auth.uid() = user_id)
    WITH CHECK (auth.uid() = user_id);
  END IF;
  
  IF NOT EXISTS (SELECT 1 FROM pg_policies WHERE tablename = 'customer_payment_preferences' AND policyname = 'Admins can view all preferences') THEN
