...
-- Update business hours to start at 8 AM and end at 6 PM
...
SET business_hours = jsonb_set(
  jsonb_set(
    ...
    '{wednesday,open}', '"08:00"'
  ),
  '{wednesday,close}', '"18:00"'
)
WHERE business_hours IS NOT NULL;

-- Update default delivery config to use 8 AM - 6 PM
UPDATE business_settings 
SET delivery_scheduling_config = jsonb_set(
  COALESCE(delivery_scheduling_config, '{}'::jsonb),
  '{default_delivery_duration_minutes}', '60'
)
WHERE delivery_scheduling_config IS NOT NULL;