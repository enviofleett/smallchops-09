-- UNIFIED PAYMENT VERIFICATION SYSTEM
...
CREATE POLICY read_intents FOR SELECT ON app.payment_intents USING (true);
CREATE POLICY read_tx FOR SELECT ON app.payment_transactions USING (true);

-- Block client from touching payment_status/reference on orders
-- Note: This will be handled by application-level restrictions
...
BEGIN
  ...
  LOOP
    ...
    SELECT 
      (SELECT id FROM app.payment_intents WHERE order_id = legacy_order.id LIMIT 1),
      legacy_order.id,
      'paystack',
      new_ref,
