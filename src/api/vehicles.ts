...
export async function createVehicle(vehicle: NewVehicle): Promise<Vehicle> {
  const { data, error } = await (supabase as any).from("vehicles").insert(vehicle).select().single();
  if (error) throw error;
  return data;
}

export async function updateVehicle(id: string, fields: Partial<Vehicle>): Promise<Vehicle> {
  const { data, error } = await (supabase as any).from("vehicles").update(fields).eq("id", id).select().single();
  if (error) throw error;
  return data;
}
...
export async function getAssignments(): Promise<Assignment[]> {
  const { data, error } = await (supabase as any).from("vehicle_assignments").select("*").order("assigned_at", { ascending: false });
  if (error) throw error;
  return data ?? [];
}
...
export async function assignVehicle(vehicle_id: string, dispatch_rider_id: string, assigned_by?: string, notes?: string) {
  ...
  const { data, error } = await (supabase as any).from("vehicle_assignments").insert([
    { vehicle_id, dispatch_rider_id, assigned_by, status: "active", notes }
  ]).select().single();
  if (error) throw error;
  // Update vehicle status
  await (supabase as any).from("vehicles").update({ status: "assigned" }).eq("id", vehicle_id);
  return data;
}
...
export async function unassignVehicle(vehicle_id: string): Promise<void> {
  await (supabase as any).from("vehicle_assignments").update({ status: "inactive" }).eq("vehicle_id", vehicle_id).eq("status", "active");
  await (supabase as any).from("vehicles").update({ status: "available" }).eq("id", vehicle_id);
}
