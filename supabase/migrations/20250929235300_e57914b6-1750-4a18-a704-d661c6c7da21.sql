-- Fix RPC - Handle unassigned records safely
-- Error: record "v_pickup_point" is not assigned yet
...
BEGIN
    ...
    -- Resolve address - safely handle v_pickup_point
    v_address := CASE
        ...
        WHEN v_order.order_type = 'pickup' THEN
            CASE
                ...
                ELSE COALESCE(v_business_settings.name || ' - Main Location', 'Pickup location to be confirmed')
            END
        ELSE 'Address not available'
    END;

    -- Build delivery hours - safely handle v_pickup_point
    ...
    -- Build comprehensive result - safely handle v_pickup_point
...
VALUES (
    'rpc_unassigned_record_fixed',
    'Database Optimization',
    'Fixed unassigned record access - use v_has_pickup_point flag',
    jsonb_build_object(
        'issue', 'Accessing v_pickup_point fields before checking if record was assigned',
        'fix', 'Added v_has_pickup_point boolean flag to safely check record assignment'
    )
);