...
-- Add unique constraint to production_checklist if it doesn't exist
DO $$ 
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_constraint 
    WHERE conname = 'production_checklist_item_name_key'
  ) THEN
    ALTER TABLE public.production_checklist 
    ADD CONSTRAINT production_checklist_item_name_key UNIQUE (item_name);
  END IF;
END $$;
