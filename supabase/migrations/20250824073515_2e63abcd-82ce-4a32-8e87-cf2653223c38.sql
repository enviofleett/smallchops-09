-- Production Email System Migration: Fix Array Format and Ensure Core Templates
-- Step 1: Add description column if needed
ALTER TABLE enhanced_email_templates 
ADD COLUMN IF NOT EXISTS description TEXT;

-- Step 2: Ensure all critical email templates exist with correct PostgreSQL array format
...
ARRAY['customer_name', 'order_number', 'order_total', 'order_date', 'store_name'],
'transactional', true
),
...
ARRAY['customer_name', 'store_name', 'support_email'],
'transactional', true
),
...
ARRAY['customer_name', 'order_number', 'order_total', 'payment_method', 'payment_reference'],
'transactional', true
),
...
-- Step 3: Deactivate duplicate templates to prevent production confusion
UPDATE enhanced_email_templates 
SET is_active = false,
    description = COALESCE(description, '') || ' [DUPLICATE - DEACTIVATED FOR PRODUCTION]'
