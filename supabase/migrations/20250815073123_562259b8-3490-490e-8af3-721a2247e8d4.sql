-- Fix ambiguous updated_at column error in verify_and_update_payment_status function
CREATE OR REPLACE FUNCTION public.verify_and_update_payment_status(payment_ref text, new_status text, payment_amount numeric DEFAULT NULL::numeric, payment_gateway_response jsonb DEFAULT NULL::jsonb)
...
BEGIN
    ...
    SET 
        ...
        updated_at = NOW(),
        paid_at = CASE WHEN new_status = 'confirmed' THEN NOW() ELSE o.paid_at END
    WHERE o.id = order_record.id
    RETURNING o.updated_at INTO v_order_updated_at;

    -- Get updated order record with explicit column references
    SELECT o.id, o.order_number, o.status, o.total_amount, o.customer_email, 
           o.order_type, o.payment_reference, v_order_updated_at
    INTO order_record
    FROM orders o
    WHERE o.id = order_record.id;
