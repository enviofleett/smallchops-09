-- Phase 1: Fix database schema and function issues

-- Step 1: Update orders table schema to match function parameters
ALTER TABLE public.orders 
  ALTER COLUMN delivery_address TYPE jsonb USING delivery_address::jsonb,
  ALTER COLUMN guest_session_id TYPE uuid USING 
    CASE 
      WHEN guest_session_id IS NULL THEN NULL
      WHEN guest_session_id ~ '^[0-9a-f]{8}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{4}-[0-9a-f]{12}$' THEN guest_session_id::uuid
      ELSE NULL
    END;

-- Step 2: Drop the old legacy create_order_with_items function that returns jsonb
DROP FUNCTION IF EXISTS public.create_order_with_items(text, text, jsonb, text, text, jsonb, uuid, text, uuid, numeric, numeric);

-- Step 3: Update the UUID-returning create_order_with_items function to fix order_type casting
...
BEGIN
  ...
  -- Create the order - FIX: Cast p_fulfillment_type to order_type enum
  INSERT INTO orders (
    order_number,
    customer_id,
    customer_name,
    ...
    p_fulfillment_type::order_type, -- FIX: Explicit cast to enum
    p_delivery_address,
    p_pickup_point_id,
    p_delivery_zone_id,
    p_guest_session_id,
