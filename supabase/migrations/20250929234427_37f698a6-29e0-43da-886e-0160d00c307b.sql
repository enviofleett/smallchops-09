-- Final fix for RPC - Use safest approach for TEXT to JSONB
-- Simply wrap all text values in a JSONB array, no risky parsing
...
BEGIN
    ...
    -- Get order items with SAFE TEXT-to-JSONB conversion
    SELECT jsonb_agg(
        jsonb_build_object(
            'id', oi.id,
            ...
            'product', jsonb_build_object(
                ...
                'ingredients', 
                    CASE 
                        WHEN p.ingredients IS NULL OR p.ingredients = '' THEN '[]'::jsonb
    ...
    v_address := CASE
        ...
        WHEN v_order.order_type = 'pickup' THEN
            COALESCE(v_pickup_point.address, v_business_settings.name || ' - Main Location', 'Pickup location to be confirmed')
    ...
    v_result := jsonb_build_object(
        ...
        'delivery_schedule', CASE WHEN v_delivery_schedule.id IS NOT NULL THEN to_jsonb(v_delivery_schedule) ELSE NULL END,
        'pickup_point', CASE WHEN v_pickup_point.id IS NOT NULL THEN to_jsonb(v_pickup_point) ELSE NULL END,
        'assigned_agent', CASE 
            ...
            ELSE NULL
        END,
        'business_settings', COALESCE(to_jsonb(v_business_settings), '{}'::jsonb),
        'fulfillment_info', jsonb_build_object(
            'type', v_order.order_type,
            'pickup_time', CASE 
                WHEN v_order.order_type = 'pickup' AND v_order.pickup_time IS NOT NULL THEN to_char(v_order.pickup_time, 'YYYY-MM-DD HH24:MI')
                ELSE NULL
            END,
            'delivery_date', CASE 
                WHEN v_order.order_type = 'delivery' AND v_delivery_schedule.delivery_date IS NOT NULL THEN to_char(v_delivery_schedule.delivery_date, 'YYYY-MM-DD')
                ELSE NULL
            END,
...
VALUES (
    ...
    'Final RPC fix - always wrap TEXT ingredients in JSONB array, no parsing',
    jsonb_build_object('fix', 'Safe TEXT to JSONB conversion without risky casting')
);