-- Fix create_order_with_items function to work with customer_accounts table
...
BEGIN
  ...
  -- Try to find existing customer account by email
  SELECT id INTO v_customer_account_id 
  FROM customer_accounts 
  WHERE name = p_customer_email OR phone = p_customer_email;
  
  -- For guest checkout, we don't create customer accounts
  -- The order will be created without a customer_id (guest order)
  -- Customer accounts are only created when users explicitly register
  ...
  -- Create the order (customer_id can be NULL for guest orders)
  INSERT INTO orders (
    order_number,
    customer_id,
    customer_name,
    ...
    v_order_number,
    v_customer_account_id, -- This will be NULL for guest orders
    p_customer_name,
    p_customer_email,
    p_customer_phone,
