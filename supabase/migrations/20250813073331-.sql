-- Temporarily disable validation trigger to fix problematic data
DROP TRIGGER IF EXISTS validate_order_update_trigger ON orders;

-- Fix the problematic orders by setting them to 'delivered' (valid transition)
UPDATE orders 
SET 
    status = 'delivered',
    updated_at = NOW()
...
-- Complete the database consistency fixes
...
-- Recreate the validation trigger with 'failed' status support
