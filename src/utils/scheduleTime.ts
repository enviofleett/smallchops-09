import { parse, isValid, addHours, format } from 'date-fns';

/**
 * BUSINESS RULE: All delivery and pickup orders use a fixed 1-hour time window.
 * The delivery_time or pickup_time field represents the START of the window.
 * 
 * Example: If delivery_time is "9:00 AM", the window is "9:00 AM - 10:00 AM"
 */

/**
 * Calculate a 1-hour time window from a start time
 * @param startTime - The start time string (e.g., "9:00 AM", "14:30")
 * @returns Object with formatted start and end times, or null if parsing fails
 */
export const calculateOneHourWindow = (
  ...
  try {
    ...
    const timeFormats = ['HH:mm', 'h:mm a', 'h:mma', 'HH:mm:ss', 'h:mm:ss a'];
    let parsedTime: Date | null = null;

    for (const formatStr of timeFormats) {
      const result = parse(startTime, formatStr, new Date());
      if (isValid(result)) {
        parsedTime = result;
        break;
      }
    ...
    return {
      start_time: format(parsedTime, 'h:mm a'),
