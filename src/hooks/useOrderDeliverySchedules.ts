...
export function EnhancedOrdersSection() {
  ...
  } = useOrderDeliverySchedules(orderIds);
  ...
  React.useEffect(() => {
    ...
    if (schedulesError) {
      ...
      handleError(schedulesError, 'loading delivery schedules');
    }
  }, [ordersError, schedulesError, handleError]);
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col lg:flex-row gap-6">
        ...
        <div className="flex-1 space-y-4">
          {orders.slice(0, 3).map((order) => {
            try {
              ...
              return (
                <Card key={`error-${order.id}`} className="p-6 border border-red-200">
        ...
        <div className="lg:w-80 w-full bg-white rounded-lg border border-gray-200 p-6">
          ...
          <div className="space-y-3">
            {orders.slice(0, 5).map((order) => {
              ...
              return (
                <div key={`summary-${order.id}`} className="flex items-center justify-between py-2 border-b border-gray-100 last:border-0">
                  <div>
