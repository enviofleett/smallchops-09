-- Fix RPC - Final Fix: customizations is JSONB, not TEXT
-- Cannot COALESCE JSONB with empty string
...
BEGIN
    ...
    -- Get order items - FINAL FIX: customizations is JSONB
    SELECT jsonb_agg(
        jsonb_build_object(
            'id', oi.id,
...
VALUES (
    'rpc_json_casting_FINALLY_fixed',
    'Database Optimization',
    'FINAL FIX: customizations column is JSONB - cannot COALESCE with empty string',
    jsonb_build_object(
        'root_cause', 'COALESCE(oi.customizations, '''') tried to convert JSONB to empty TEXT string',
        'fix', 'Changed to COALESCE(oi.customizations, {}::jsonb)'
    )
);