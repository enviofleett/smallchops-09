-- CRITICAL PRODUCTION FIXES FOR DUPLICATE KEY VIOLATIONS - CORRECTED
...
-- 2. Enhanced dedupe key generation with atomic guarantees
...
-- 3. Circuit breaker for non-blocking email events
...
-- 4. Enhanced order status update with bulletproof email handling
...
-- 5. Add performance indexes for faster conflict detection (without immutable function issues)
...
CREATE INDEX IF NOT EXISTS idx_communication_events_deduplication_recent
ON communication_events (order_id, event_type, recipient_email, created_at DESC);

-- 6. Cleanup function for old events (prevent table bloat)
