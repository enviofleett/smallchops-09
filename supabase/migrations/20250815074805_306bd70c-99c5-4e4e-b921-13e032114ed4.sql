...
-- Create orders_with_payment view for the frontend hooks
...
SELECT 
    ...
    o.paid_at,
    o.order_type,
    -- Computed payment status based on multiple factors
    CASE 
        WHEN o.payment_status = 'paid' THEN 'paid'
        WHEN o.status = 'confirmed' AND o.paid_at IS NOT NULL THEN 'paid'
        WHEN pt.status = 'success' OR pt.status = 'paid' THEN 'paid'
        WHEN pt.status = 'failed' THEN 'failed'
        WHEN pt.status = 'pending' THEN 'pending'
        WHEN o.payment_status = 'pending' THEN 'pending'
        WHEN o.payment_reference IS NOT NULL THEN 'processing'
        ELSE 'pending'
...
LEFT JOIN payment_transactions pt ON (
    pt.provider_reference = o.payment_reference 
    OR pt.provider_reference = o.paystack_reference
    OR pt.order_id = o.id
);

-- Grant access to the view
GRANT SELECT ON private.orders_with_payment TO authenticated, service_role;