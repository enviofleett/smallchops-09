-- Backfill and force-confirm two recent orders using canonical Paystack references (adjusted columns)
DO $$
DECLARE
...
BEGIN
  ...
  IF v_txn1_id IS NULL THEN
    INSERT INTO payment_transactions (
      order_id, transaction_type, amount, currency, payment_method, provider_reference,
      status, paid_at, channel, gateway_response, processed_at, provider_response,
      metadata, created_at, updated_at
    ) VALUES (
      v_order1_id, 'charge', v_ord1_amount, 'NGN', 'paystack', v_ref1,
      'paid', v_now, 'online', 'Backfill force-confirm via admin request', v_now, '{}'::jsonb,
      jsonb_build_object('order_number', v_ord1_number, 'canonical_reference', v_ref1, 'backfill', true), v_now, v_now
    );
  ELSE
  ...
  IF v_txn2_id IS NULL THEN
    INSERT INTO payment_transactions (
      ...
      v_order2_id, 'charge', v_ord2_amount, 'NGN', 'paystack', v_ref2,
      'paid', v_now, 'online', 'Backfill force-confirm via admin request', v_now, '{}'::jsonb,
      jsonb_build_object('order_number', v_ord2_number, 'canonical_reference', v_ref2, 'backfill', true), v_now, v_now
    );
  ELSE
    UPDATE payment_transactions
    SET 
