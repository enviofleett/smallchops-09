-- Phase 4: Database Schema Updates - Change acquired_by from text to UUID

-- First, update existing records to use proper UUIDs (if any exist)
UPDATE order_update_locks 
SET acquired_by = (
  SELECT id::text 
  FROM profiles 
  WHERE profiles.id::text = order_update_locks.acquired_by 
  LIMIT 1
)
...
VALUES (
  'lock_system_schema_updated',
  'System Migration',
  'Updated lock system to use UUID for acquired_by column',
  jsonb_build_object(
    'migration_type', 'schema_update',
    'changes', 'acquired_by: text -> UUID, updated functions'
  )
);