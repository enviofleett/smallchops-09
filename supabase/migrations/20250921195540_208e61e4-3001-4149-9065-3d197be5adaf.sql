-- Fix create_order_with_items function signature with proper parameter ordering
-- All required parameters first, then optional ones with defaults

-- Drop the existing function
DROP FUNCTION IF EXISTS public.create_order_with_items(uuid, text, jsonb, uuid, uuid, uuid, jsonb, text, numeric);

-- Recreate function with PostgreSQL-compliant parameter order
...
BEGIN
    ...
    -- Calculate subtotal from items with consistent rounding
    FOR v_item IN SELECT * FROM jsonb_array_elements(p_items)
    LOOP
        v_subtotal := v_subtotal + ROUND(((v_item->>'unit_price')::numeric * (v_item->>'quantity')::integer) * 100) / 100;
    END LOOP;
    ...
    -- Enhanced tolerance-based validation (5 naira tolerance)
    IF p_client_total IS NOT NULL THEN
        IF ABS(v_total_amount - p_client_total) > 5 THEN
            RAISE LOG 'Order total mismatch - Server: %, Client: %, Diff: %, Tolerance exceeded', 
        ...
        ELSIF ABS(v_total_amount - p_client_total) > 0.01 THEN
            RAISE LOG 'Minor total discrepancy (%.2f naira) within tolerance - using server total: %', 
    ...
    INSERT INTO orders (
        ...
        order_number, fulfillment_type, delivery_address, pickup_point_id,
        delivery_zone_id, guest_session_id, subtotal_amount, delivery_fee,
        discount_amount, delivery_discount, total_amount, status,
        payment_status, promotion_code
    ) VALUES (
        p_customer_id, v_customer_email, v_customer_name, v_customer_phone,
        v_order_number, p_fulfillment_type::order_fulfillment_type, p_delivery_address, p_pickup_point_id,
        p_delivery_zone_id, p_guest_session_id, v_subtotal, v_delivery_fee,
        v_discount_amount, v_delivery_discount, v_total_amount, 'pending'::order_status,
        'pending'::payment_status, p_promotion_code
    )
    ...
    LOOP
        INSERT INTO order_items (
            ...
            ROUND(((v_item->>'unit_price')::numeric * (v_item->>'quantity')::integer) * 100) / 100,
    ...
    RAISE LOG 'Order created successfully: ID=%, Total=%', v_order_id, v_total_amount;
    RETURN v_order_id;
