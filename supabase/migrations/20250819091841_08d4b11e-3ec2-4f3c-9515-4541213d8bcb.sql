-- Database hardening for delivery schedule persistence (fix for existing policies)

-- Drop existing conflicting policies if they exist and recreate them
DROP POLICY IF EXISTS "Customers can view their own delivery schedules" ON public.order_delivery_schedule;
