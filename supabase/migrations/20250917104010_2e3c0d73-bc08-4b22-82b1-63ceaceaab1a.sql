...
-- Remove legacy MailerSend columns if they exist (corrected)
DO $$ 
BEGIN
...
-- Ensure all communication_events have proper status (using correct enum values)
UPDATE communication_events 
SET status = 'failed'
WHERE status NOT IN ('queued', 'processing', 'sent', 'failed', 'bounced');
