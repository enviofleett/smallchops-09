-- FINAL FIX: Correct column names for orders table
...
BEGIN
    ...
    GROUP BY o.id, o.order_number, o.customer_name, o.customer_email, o.customer_phone, 
             o.delivery_address, o.total_amount, o.tax_amount, o.status, 
...
SELECT 'DATABASE_FUNCTION_TEST' as test_type, 
       CASE WHEN (SELECT get_detailed_order_with_products((SELECT id FROM orders LIMIT 1))::text) LIKE '%error%' 
            THEN 'FAILED' 
            ELSE 'PASSED' 
       END as result;