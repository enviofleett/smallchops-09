-- Fix invalid JSON casting in get_comprehensive_order_fulfillment RPC function
-- This migration fixes the PostgreSQL 22P02 error caused by double casting JSONB fields
...
BEGIN
    ...
    -- Get order items with complete product details (FIXED JSON CASTING)
    SELECT jsonb_agg(
        jsonb_build_object(
            'id', oi.id,
    ...
    -- Build comprehensive result using to_jsonb instead of row_to_json
...
VALUES (
    'rpc_json_casting_fixed',
    'Database Optimization',
    'Fixed invalid JSON casting in get_comprehensive_order_fulfillment RPC function',
    jsonb_build_object(
        'fixed_fields', jsonb_build_array('features', 'ingredients'),
        'error_prevented', '22P02 - invalid input syntax for type json',
        'impact', 'Order items will now display correctly in production'
    )
);