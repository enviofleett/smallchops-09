...
import { RefreshCw, ShoppingBag, Clock, CheckCircle, AlertTriangle } from 'lucide-react';
...
function OrdersContent() {
  ...
  const ordersQuery = useCustomerOrdersOptimized();
  ...
  useEffect(() => {
    ...
    console.log('📡 Setting up realtime subscription for:', user.email);

    const channel = supabase
      .channel(`orders:${user.email.replace('@', '_').replace('.', '_')}`)
      .on(
        'postgres_changes',
        {
          event: '*',
        ...
        (payload) => {
          ...
          const oldStatus = (payload as any)?.old?.payment_status;
          const newStatus = (payload as any)?.new?.payment_status;
          if (oldStatus !== 'paid' && newStatus === 'paid') {
            window.dispatchEvent(new CustomEvent('payment-confirmed', {
              detail: { orderId: (payload as any)?.new?.id }
            }));
          }
        }
      )
  ...
  if (!isAuthenticated) {
    return (
      <div className="text-center p-8 bg-info/5 border border-info/20 rounded-lg">
        <AlertTriangle className="mx-auto h-12 w-12 text-info mb-4" />
