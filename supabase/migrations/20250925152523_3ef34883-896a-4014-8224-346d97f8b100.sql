-- Fix all critical issues in get_comprehensive_order_fulfillment function
...
BEGIN
    ...
    -- Aggregate special instructions from all sources
    v_special_instructions := TRIM(COALESCE(
        ...
        END ||
        CASE 
            WHEN v_order.delivery_instructions IS NOT NULL AND LENGTH(TRIM(v_order.delivery_instructions)) > 0
            THEN CASE WHEN (v_delivery_schedule.special_instructions IS NOT NULL OR v_order.special_instructions IS NOT NULL) THEN ' | ' || v_order.delivery_instructions ELSE v_order.delivery_instructions END
            ELSE NULL
        END,
    ...
    -- Build comprehensive result with enhanced pickup-specific logic
    v_result := jsonb_build_object(
        ...
        'fulfillment_info', jsonb_build_object(
            ...
            'pickup_point_phone', v_pickup_point.phone,
