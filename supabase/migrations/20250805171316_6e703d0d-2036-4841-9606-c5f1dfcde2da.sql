-- CRITICAL PRODUCTION FIX: Completely remove all versions of create_order_with_items
-- Get all function signatures first
DO $$
DECLARE
    func_record RECORD;
BEGIN
    -- Drop all versions of create_order_with_items by iterating through them
    FOR func_record IN 
    ...
    LOOP
        EXECUTE format('DROP FUNCTION IF EXISTS public.%s(%s)', func_record.proname, func_record.args);
