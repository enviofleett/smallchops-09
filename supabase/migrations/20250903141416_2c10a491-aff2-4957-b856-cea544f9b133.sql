...
-- 1) Atomic start_delivery: assign rider and move order to out_for_delivery in one transaction
...
BEGIN
  ...
  IF NOT is_admin() THEN
    RAISE EXCEPTION 'Only admins can start delivery';
  END IF;
  ...
  -- Return compact result (avoid leaking full PII)
...
-- 2) Reassign rider while order is already in transit (or still before pickup)
