-- Production-ready Paystack integration hardening
...
CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_payment_reference_unique 
ON public.orders (payment_reference) 
WHERE payment_reference IS NOT NULL;

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_idempotency_key_unique 
ON public.orders (idempotency_key) 
WHERE idempotency_key IS NOT NULL;

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_payment_transactions_idempotency_key_unique 
ON public.payment_transactions (idempotency_key) 
WHERE idempotency_key IS NOT NULL;

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_payment_transactions_provider_reference_unique 
ON public.payment_transactions (provider_reference) 
WHERE provider_reference IS NOT NULL;

CREATE UNIQUE INDEX CONCURRENTLY IF NOT EXISTS idx_payment_transactions_webhook_event_id_unique 
ON public.payment_transactions (webhook_event_id) 
WHERE webhook_event_id IS NOT NULL;

-- 4. Add performance indexes for common queries
CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_orders_status_payment_reference 
ON public.orders (status, payment_reference) 
WHERE payment_reference IS NOT NULL;

CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_payment_transactions_status_created_at 
...
BEGIN
  ...
  FOR UPDATE NOWAIT; -- Prevent concurrent processing
