-- Fix RPC JSON casting error (CORRECTED VERSION)
-- The ingredients column is TEXT type, not JSONB, so we need proper text-to-jsonb casting
...
BEGIN
    ...
    -- Get order items with complete product details (FIXED JSON CASTING FOR TEXT COLUMNS)
    SELECT jsonb_agg(
        jsonb_build_object(
            'id', oi.id,
            ...
            'product', jsonb_build_object(
                ...
                'ingredients', 
                    CASE 
                        ...
                        WHEN p.ingredients ~ '^[\[\{]' THEN 
                            CASE 
                                WHEN p.ingredients::jsonb IS NOT NULL THEN p.ingredients::jsonb
                                ELSE '[]'::jsonb
                            END
    ...
    -- Build comprehensive result using to_jsonb
...
-- Log the corrected fix
INSERT INTO audit_logs (action, category, message, new_values)
VALUES (
    'rpc_json_casting_fixed_v2',
    'Database Optimization',
    'Fixed RPC function - ingredients is TEXT type, not JSONB',
    jsonb_build_object(
        'issue', 'ingredients column is TEXT, not JSONB',
        'fix', 'Added proper TEXT to JSONB conversion with regex validation',
        'error_prevented', '22P02 - invalid input syntax for type json'
    )
);