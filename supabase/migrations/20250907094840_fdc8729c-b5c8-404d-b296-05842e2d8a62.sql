-- Fix email system production readiness issues (Final version with correct template_type)
...
-- 3. Seed essential email templates for production (with correct template_type)
...
DROP TRIGGER IF EXISTS trigger_order_status_email_notifications ON orders;
...
-- Create the trigger
CREATE TRIGGER trigger_order_status_email_notifications
  AFTER UPDATE ON orders
  FOR EACH ROW
  EXECUTE FUNCTION trigger_order_status_email_notifications();