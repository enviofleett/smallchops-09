-- CRITICAL PRODUCTION FIX: Resolve checkout database function errors
-- Drop all existing conflicting versions of create_order_with_items
DROP FUNCTION IF EXISTS public.create_order_with_items(text, text, jsonb, jsonb);
DROP FUNCTION IF EXISTS public.create_order_with_items(text, text, text, jsonb, jsonb, text, text, uuid);
DROP FUNCTION IF EXISTS public.create_order_with_items(text, text, text, jsonb, jsonb, text, text, text);

-- Create the unified, properly working function
...
BEGIN
    ...
    BEGIN
        ...
        -- Create order with correct column mapping (fulfillment_type -> order_type)
        INSERT INTO public.orders (
            customer_id,
            customer_email,
            customer_name,
            customer_phone,
            order_type,  -- FIXED: Map p_fulfillment_type to order_type column
            delivery_address,
            total_amount,
            status,
            payment_status,
    ...
    EXCEPTION 
        ...
        WHEN OTHERS THEN
            RAISE LOG 'Order creation failed with error: % (SQLSTATE: %)', SQLERRM, SQLSTATE;
...
SELECT p.proname as function_name, p.proargnames as argument_names
FROM pg_proc p
