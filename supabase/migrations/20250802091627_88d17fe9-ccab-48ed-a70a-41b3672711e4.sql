...
-- 1. Drop triggers that depend on the trigger_customer_welcome_email function
DROP TRIGGER IF EXISTS customer_welcome_email ON public.customer_accounts;
DROP TRIGGER IF EXISTS on_customer_account_created ON public.customer_accounts;

-- 2. Drop the outdated trigger_customer_welcome_email function
DROP FUNCTION IF EXISTS public.trigger_customer_welcome_email() CASCADE;

-- 3. Create a proper trigger function for new customer registrations
...
-- 4. Create trigger on customers table for automatic welcome emails
...
-- 5. Update create_customer_with_validation function to use consistent event types
...
-- 6. Fix get_hourly_email_stats function to resolve column ambiguity
...
-- 7. Add function to requeue failed welcome emails after SMTP is fixed
