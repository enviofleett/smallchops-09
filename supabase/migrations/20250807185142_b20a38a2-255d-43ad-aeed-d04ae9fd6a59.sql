...
-- Fix leaked password protection (enable password strength checking)
ALTER ROLE authenticator SET app.settings.jwt_secret TO ''; -- This will be set by Supabase
-- Enable password strength checking in auth settings
UPDATE auth.config SET password_requirements = jsonb_build_object(
  'min_length', 8,
...
-- Create the missing check_paystack_production_readiness function
