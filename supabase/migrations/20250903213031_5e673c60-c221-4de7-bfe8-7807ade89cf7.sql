...
-- Fix: correct the payment_status comparison in trigger_order_emails
...
BEGIN
  ...
  IF TG_OP = 'INSERT' THEN
    -- Look up customer details if available
    SELECT 
      NEW.*,
  ...
  ELSIF TG_OP = 'UPDATE' THEN
    ...
    -- Payment confirmation using idempotent RPC when payment_status changes to 'paid'
    IF OLD.payment_status IS DISTINCT FROM NEW.payment_status
      ...
      -- Log the issue without failing the order update
      IF (confirmation_result->>'success')::boolean = false THEN
...
EXCEPTION WHEN OTHERS THEN
  -- Log error but don't block order creation/update
  INSERT INTO audit_logs (
    action, category, message, entity_id, new_values
  ) VALUES (
